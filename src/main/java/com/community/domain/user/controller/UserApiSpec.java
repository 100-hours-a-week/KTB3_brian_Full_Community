...
public interface UserApiSpec {
    ...
    ResponseEntity<ApiResponse<SignInResponse>> signIn(@Valid SignInRequest req);

    @Operation(summary = "회원가입 이메일 검증", description = "이메일 사용 가능 여부를 확인합니다.")
    ResponseEntity<ApiResponse<SignInAvailableResponse>> checkEmailAvailability(@NotBlank String email);

    @Operation(summary = "회원가입 닉네임 검증", description = "닉네임 사용 가능 여부를 확인합니다.")
    ResponseEntity<ApiResponse<SignInAvailableResponse>> checkNicknameAvailability(@NotBlank String nickname);
